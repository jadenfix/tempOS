...
This is a sequencing decision, not a product ceiling. beaterOS should keep three explicit engineering lanes:

1. **Hosted compatibility lane.** Build the agent kernel, policy engine, sandbox lane, memory provenance, scenario runner, receipts, payment mandates, and audit tools on macOS, Linux, containers, browsers, and cloud VMs. This lane is the always-runnable contract and evidence generator.
2. **Linux add-on lane.** Use Linux-native mechanisms as optional accelerators for learning and deployment: cgroups, namespaces, seccomp, LSMs, eBPF/XDP, sched_ext, io_uring, DPDK, SPDK, KVM, microVMs, and Rust-for-Linux or C kernel boundaries where they provide measurable wins. Linux add-ons must preserve the same beaterOS authority, receipt, telemetry, fallback, and macOS abstraction story.
3. **Metal-touching OS lane.** Build the smallest new low-level OS pieces that agent workloads actually require: capability-secure task admission, policy-aware scheduling, bounded memory and context management, zero-copy trace and receipt paths, accelerator queue/residency management, high-assurance sandboxing, and recoverable journals. This lane may pass through a simulator, a microkernel appliance, a library OS, a hypervisor-backed runtime, or a RISC-V/ARM research target before general hardware support.
...
- **Vendor-neutral silicon contract.** CUDA, Metal, XLA/TPU, LPU providers, NPUs, Apple Neural Engine/Core ML-style framework routing, media engines, and enclaves are backend implementations. The OS contract is admission, queue bounds, residency policy, cancellation, telemetry, receipts, fallback, and replay.
